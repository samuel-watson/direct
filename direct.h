...
template <typename T>
class direct {
 using func = T(*)(const void*, std::vector<T>&);
...
public:
 ...
 auto    operator()(std::vector<T>& vec) const -> T;
 void    optim();
 std::vector<T> values() const;
 
private:
 [[noreturn]]
 static auto null_fn(const void* p, std::vector<T> vec) -> T
 {
   throw bad_fn{};
 }
...
inline void direct<T>::fn() 
{
 ...
 optim_fn = static_cast<func>([](const void*, std::vector<T>& vec) -> T {
...
template <typename T>
inline auto direct<T>::operator()(std::vector<T>& vec) const -> T
{
...
inline void direct<T>::optim(){
#ifdef R_BUILD
 Rcpp::Rcout << "\nSTARTING DIRECT-L";
 Rcpp::Rcout << "\nTolerance: " << tol << " | Max iter : " << max_iter << "\n Starting values :";
 ...
 while(max_diff > tol && iter <= max_iter){
   ...
   if(iter < max_iter)divide_rectangles();
#ifdef R_BUILD
...
template <typename T>
inline auto direct<T>::eval(std::vector<T>& vec) -> T
{   
 fn_counter++;
 return std::invoke(optim_fn,optim_instance,vec);
}  
